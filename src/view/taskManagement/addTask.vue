    ...
    <Top>
      <div class="top-left" slot="breadcrumb">
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item>
            <img class="appManage" src="@/assets/images/file.png" />新增课程
          </el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    </Top>
        ...
        <el-form ref="form" :model="form" label-width="80px" class="bottom-form">
          ...
          <el-form-item label="课程属性：">
            <el-radio-group v-model="addtype">
              <el-radio @change="changeSelect2" :label="item.typeId" v-for="item in allType2" :key="item.typeId">
                {{item.typeName}}
                </el-radio>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item v-if="addtype == 1" label="佣金：">
            <el-input v-model="spYj" class="inp2 input3"></el-input>
            <span>佣金比例：</span>
            <el-input v-model="spYjbl" class="inp2 input3"></el-input>
            <span>%</span>
          </el-form-item>
          <el-form-item v-if="addtype == 1" label="商品标题">
            <el-input v-model="spName" class="inp2 input3"></el-input>
          </el-form-item>
          <el-form-item v-if="addtype == 1" label="商品编号：">
            <el-input v-model="form.subjectMatterId" class="inp input3"></el-input>
            <span>商品原价：</span>
            <el-input v-model="spPrice" class="inp2 input3"></el-input>
            <span>商品返佣：</span>
            <el-input v-model="spFy" class="inp2 input3"></el-input>
          </el-form-item>
          ...
          <el-form-item :label="addtype == 1?'商品图片：':'课程图片：'">
            <el-upload class="bannerBg" :action="uploadActionUrl" :show-file-list="false"
              ...
              <div v-if="!imageUrl" class="uploadWen">尺寸：500px * 275px</div>
            </el-upload>
          </el-form-item>
          <el-form-item label="课程视频：" v-for="(v, index) in form.videoinfos" :key="v.key"
            ...
            <el-upload class="upload-demo" :action="videoActionUrl" :on-preview="handlePreviewVideo" :data="extraParams"
              ...
              :on-success="(file, fileList) => {handleAvatarSuccessVideo(index,file, fileList)}"
              :file-list="v.videoArr">
    ...
    methods: {
      ...
      async addTask() {
        ...
        if(this.addtype ==1){
          // 带货
          var params1 = {
            commissionShare:this.spYjbl,
            commissonPrice:this.spYj,
            id:this.form.subjectMatterId,
            imgUrl:this.form.imageUrl,
            materialUrl:this.form.promoteUri,
            price:this.spPrice,
            skuName:this.spName,
          }
          await addInfo(params1).then(res => {
            if (res.code == 1) {
              this.addKc();
            }
          });
...
<style lang="less" scoped>
  ...
  .bottom {
    ...
    /deep/.bottom-form .inp2 .el-input__inner {
      .wh(100px, 40px);
      display: inline-block;
    }
