  ...
  <div class="task-list">
    <Top>
      ...
      <div class="top-right" slot="btn">
        <el-button class="btn left-btn" @click="updateCustomer">保存</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      getData() {
        this.imageUrl = this.$route.params.row.qrcode;
        this.officialName = this.$route.params.row.officialName;
        this.describe = this.$route.params.row.describe;
      },
      async updateCustomer() {
        ...
        await updateCustomer(params).then(res => {
          if (res.code == 1) {
            this.getData();
            this.$message({
              message: "编辑成功",
